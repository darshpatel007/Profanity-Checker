...
url = "http://api1.webpurify.com/services/rest/?method=webpurify.live.return&api_key="+webpurify_api_key+"&format=json&text="+str(text)
response = requests.get(url).json()

try :
    
    no_of_offensive_words = int(response['rsp']['found'])
    print("Total Offensive Words Found : ",no_of_offensive_words)
    
    if no_of_offensive_words == 0 :
        print("No Offensive Words Found")    
    else :
        offensive_words = response['rsp']['expletive']
